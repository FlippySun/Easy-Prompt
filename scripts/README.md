...
| 脚本 | 说明 |
|------|------|
| `backend-dev.sh` | 启动后端 dev 服务器（可选 `--with-tunnel` 自动建立 SSH 隧道） |
| `backend-build.sh` | 编译 TypeScript → dist/（可选 `--clean`） |
| `backend-test.sh` | 运行测试（`--mode=unit\|integration\|coverage\|all`） |
| `backend-deploy.sh` | 部署到 VPS（rsync + pm2 reload，支持 `--dry-run`） |
| `backend-tunnel.sh` | SSH Tunnel 管理（`start\|stop\|status\|restart`） |
| `backend-db.sh` | Prisma 数据库操作（`migrate\|seed\|studio\|generate\|reset`） |
...
# 启动后端开发（含 SSH tunnel）
./scripts/backend-dev.sh --with-tunnel
