...
- **多端同步（最高优先级）**：任何修改必须同步到所有已注册平台端（VSCode / IntelliJ / 未来新端），详见 `.github/copilot-instructions.md` 规则 4「多端同步协议」
- 修改 core/ 时，IntelliJ 端有独立 Kotlin 实现（Scenes.kt/Router.kt/ApiClient.kt），必须手动同步
- Bug 修复时必须检查其他端是否存在相同 Bug
