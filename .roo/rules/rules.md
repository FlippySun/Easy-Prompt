...
## 项目结构
```
core/           → 平台无关的共享逻辑（scenes, router, composer, api）
vscode/         → VSCode 扩展封装
...
# 语法检查
node --check vscode/extension.js && node --check core/index.js
```
...
| `core/api.js` | curl subprocess API |
| `vscode/extension.js` | VSCode 扩展入口 |
| `vscode/welcomeView.js` | Welcome 引导页 |
