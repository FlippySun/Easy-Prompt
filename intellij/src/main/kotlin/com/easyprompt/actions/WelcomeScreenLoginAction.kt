...
// [日期]     2026-04-10
// [类型]     修复
// [描述]     调整 IDEA 启动 Welcome Screen 的 PromptHub CTA：登录后展示当前账号，并改为点击直达 Web-Hub 个人页。
// [思路]     未登录时继续复用 SsoAuthClient.startLogin()；已登录时改为打开当前运行环境的个人主页，与 ToolWindow 欢迎页 CTA 语义保持一致。
...
// [参数与返回值] actionPerformed(e) 已登录时打开当前运行环境的 Web-Hub 个人主页，未登录时继续发起 SSO；update(e) 同步刷新按钮说明文案。
...
class WelcomeScreenLoginAction : AnAction() {
    ...
    override fun update(e: AnActionEvent) {
        ...
        e.presentation.text = if (isLoggedIn) "当前登录用户：$accountName" else "登录 PromptHub"
        e.presentation.description = if (isLoggedIn) {
            "当前已登录：$accountName。点击可直接打开当前环境个人主页。"
        } else {
            "在 IDEA Welcome Screen 中直接发起 PromptHub SSO 登录。"
        }
    }
}
