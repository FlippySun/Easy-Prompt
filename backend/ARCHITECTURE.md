...
function getErrorMessage(code: string, lang: "zh" | "en" = "zh"): string {
  return (
    ...
    ERROR_MESSAGES["SYSTEM_INTERNAL_ERROR"][lang]
  );
}
...
class AppError extends Error {
  ...
  constructor(
    ...
    this.code = code;
    this.httpStatus =
      options?.httpStatus ?? DEFAULT_STATUS[code.split("_")[0]] ?? 500;
...
```
  ...
  Internet ────────→│  Nginx (443/80)                            │
                    ...
                    │    ├── zhiz.chat/api/* → localhost:3001    │ (后端 API 反代)
                    │    ├── prompt.zhiz.chat → /www/.../web/    │ (Web SPA 静态)
                    │    ├── prompt.zhiz.chat/api/* → :3001      │ (Web 端也走后端)
                    │    └── api.zhiz.chat/* → localhost:3001    │ (⭐ 全端统一 API)
                    │                                            │
...
注：prompt.zhiz.chat/ep-api/* (现有 Nginx 直连第三方代理) 将在全端迁移完成后废弃
```
