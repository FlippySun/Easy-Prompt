...
- [ ] 删除 `browser/options/options.html` 旧文件，完成 WXT 迁移清理
- [ ] 验证 Chrome / Firefox / Safari 三浏览器 WXT 构建正常
- [ ] 新增 Microsoft Edge 浏览器支持（Chromium 内核，与 Chrome 类似）
- [ ] WXT 配置中为 Edge 添加 manifest browser_specific_settings
- [ ] 添加 Edge 应用商店发布配置
- [ ] 建立统一测试运行器（Vitest）
- [ ] 添加 browser/shared 共享模块的单元测试
- [ ] 建立浏览器扩展端到端测试（Playwright + Chrome/Firefox/Edge）
- [ ] 添加跨平台 parity 测试（browser vs core vs web）
- [ ] CI 集成测试运行
- [ ] 更新 `test.js` 场景数量警告（85 → 97）
