    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-30 flex flex-col items-end gap-2">
          ...
          {expanded && (
            <>
              ...
              >
                <ArrowLeftRight
                  size={15}
                  className={store.compare.length === 2 ? 'text-violet-500' : 'text-gray-400'}
                />
                对比 Prompt
        ...
        <MagneticButton strength={28}>
          <motion.button
            ...
            whileTap={{ scale: 0.93 }}
            className="relative flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br from-violet-500 to-indigo-600 shadow-xl shadow-indigo-500/30 transition-all"
          >
      ...
      <AnimatePresence>
        {hasCompareItems && !expanded && (
          ...
          >
            <p
              className={`px-1 text-[10px] font-semibold uppercase tracking-widest ${dm ? 'text-gray-500' : 'text-gray-400'}`}
            >
            ...
            {store.compare.map((id, i) => {
              ...
              return (
                <div
                  key={id}
                  className={`flex items-center gap-2 rounded-xl border p-2 ${dm ? 'border-gray-800 bg-gray-800/60' : 'border-gray-100 bg-gray-50'}`}
                >
                  <span
                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md text-[9px] font-black text-white ${i === 0 ? 'bg-indigo-500' : 'bg-pink-500'}`}
                  >
                    {i === 0 ? 'A' : 'B'}
                  </span>
                  <span
                    className={`flex-1 truncate text-[11px] font-medium ${dm ? 'text-gray-300' : 'text-gray-700'}`}
                    style={{ maxWidth: 140 }}
                  >
                    {p.title}
                  </span>
                  <button
                    ...
                    className={`shrink-0 transition-colors ${dm ? 'text-gray-600 hover:text-gray-400' : 'text-gray-300 hover:text-gray-600'}`}
                  >
                    <X size={11} />
                  </button>
                </div>
            ...
            {store.compare.length === 2 && (
              <button
                onClick={handleCompare}
                className="mt-1 flex w-full items-center justify-center gap-1.5 rounded-xl bg-linear-to-r from-violet-500 to-indigo-600 py-2 text-xs font-semibold text-white"
              >
      ...
      {/* Compare Modal */}
      {compareOpen && <CompareModal compareIds={store.compare} darkMode={dm} onClose={() => setCompareOpen(false)} />}
    </>
  );
}
