...
export function MagneticButton({ children, strength = 25, className }: MagneticButtonProps) {
  ...
  const animate = useCallback(() => {
    ...
    if (contentRef.current) {
      contentRef.current.style.transform = `translate(${currentRef.current.x}px, ${currentRef.current.y}px)`;
    }
  ...
  return (
    <div ref={wrapRef} className={className} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave}>
