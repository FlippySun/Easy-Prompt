    ...
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-linear-to-br from-yellow-400 to-orange-500">
      ...
      {/* Tab Bar */}
      <div
        className={`flex items-center gap-1 rounded-xl border p-1 self-start ${dm ? 'border-gray-800 bg-gray-900' : 'border-gray-200 bg-gray-100'}`}
      >
        {TABS.map(({ id, label }) => (
          <button
            key={id}
            onClick={() => setTab(id as 'saved' | 'liked')}
      ...
      <AnimatePresence mode="wait">
        {ids.size === 0 ? (
          ...
          >
            <div
              className={`mb-6 flex h-24 w-24 items-center justify-center rounded-full ${dm ? 'bg-gray-800' : 'bg-gray-100'}`}
            >
            ...
            <Link
              to="/"
              className="flex items-center gap-2 rounded-xl bg-linear-to-r from-violet-500 to-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-md"
            >
              探索 Prompt 库
            </Link>
          </motion.div>
        ) : (
          <motion.div key={tab} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-4">
            {/* Filter Row */}
            <div className="flex items-center gap-3">
              ...
              <span className={`text-sm ${dm ? 'text-gray-500' : 'text-gray-400'}`}>{prompts.length} 个结果</span>
              {ids.size > 0 && (
                <button
                      ...
                      <motion.div
                        ...
                        className={`overflow-hidden rounded-2xl border transition-all duration-200 ${
                          dm
                            ...
                            : 'border-gray-200/80 bg-white hover:border-indigo-200 hover:shadow-lg'
                        }`}
                      >
                        <div
                        ...
                        <div className="p-4 flex flex-col gap-3">
                          ...
                          <button
                            ...
                            className={`text-left text-[15px] font-semibold leading-snug hover:text-indigo-500 transition-colors ${dm ? 'text-gray-100' : 'text-gray-900'}`}
                          >
                            {prompt.title}
                          </button>
                          ...
                          {/* Description */}
                          <p
                            className={`text-sm leading-relaxed line-clamp-2 ${dm ? 'text-gray-400' : 'text-gray-500'}`}
                          >
                            {prompt.description}
                          </p>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {prompt.tags.slice(0, 3).map((tag) => (
                              <span
                                key={tag}
                                className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${dm ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'}`}
                              >
                                #{tag}
                              </span>
                            ))}
                          ...
                          {/* Footer */}
                          <div
                            className={`flex items-center justify-between border-t pt-3 ${dm ? 'border-gray-800' : 'border-gray-100'}`}
                          >
                            ...
                            <div className="flex gap-1.5">
                              <button
                                onClick={() => {
                                  ...
                                  toast.success(wasSaved ? '已移出收藏' : '已添加收藏');
                                }}
