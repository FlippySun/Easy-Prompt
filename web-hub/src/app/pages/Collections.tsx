  ...
  return (
    <motion.div
      ...
      className={`group relative flex cursor-pointer flex-col overflow-hidden rounded-2xl border transition-all duration-300 ${
        dm
          ? 'border-gray-800 bg-gray-900 hover:border-gray-700 hover:shadow-xl hover:shadow-black/20'
          : 'border-gray-200/80 bg-white hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-500/8'
      }`}
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <span
            ...
            style={{
              background: dm ? diffConfig.darkBg : diffConfig.bg,
              color: dm ? diffConfig.darkColor : diffConfig.color,
            }}
          >
        ...
        <p className={`text-xs leading-relaxed ${dm ? 'text-gray-400' : 'text-gray-500'}`}>{collection.description}</p>
        ...
        <div className="flex flex-wrap gap-1">
          {collection.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className={`rounded-md px-2 py-0.5 text-[10px] font-medium ${dm ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'}`}
            >
              #{tag}
            </span>
          ))}
        ...
        {/* Footer */}
        <div
          className={`mt-auto flex items-center justify-between border-t pt-3 ${dm ? 'border-gray-800' : 'border-gray-100'}`}
        >
          ...
          <div
            className={`flex items-center gap-1 text-[11px] font-semibold transition-colors group-hover:text-indigo-500 ${dm ? 'text-gray-400' : 'text-gray-500'}`}
          >
            查看合集
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold ${dm ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'}`}
                  >
                    {i + 1}
                  </span>
              ...
              <div
                className={`flex items-center justify-between border-t pt-2.5 ${dm ? 'border-gray-800' : 'border-gray-100'}`}
              >
                <div className="flex flex-wrap gap-1">
                  {prompt.tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className={`rounded-md px-1.5 py-0.5 text-[10px] ${dm ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'}`}
                    >
                      #{tag}
                    </span>
                  ))}
          ...
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-linear-to-br from-indigo-400 to-violet-500">
              ...
              <div className="relative max-w-xs flex-1">
                <Search
                  size={14}
                  className={`absolute left-3 top-1/2 -translate-y-1/2 ${dm ? 'text-gray-500' : 'text-gray-400'}`}
                />
                <input
                  type="text"
                  value={search}
            ...
            >
              {[
                ...
                <div key={label} className="flex items-center gap-2">
                  ...
                  <div>
                    <p className={`text-base font-bold ${dm ? 'text-white' : 'text-gray-900'}`}>
