...
function loadStore(): PromptStore {
  try {
    ...
    return {
      ...
      savedCollections: new Set(parsed.savedCollections || []),
  ...
  try {
    localStorage.setItem(
      ...
      JSON.stringify({
        liked: [...store.liked],
        saved: [...store.saved],
        savedCollections: [...store.savedCollections],
        copied: store.copied,
        viewed: store.viewed,
...
// Batch notifications via microtask to avoid cascading re-renders
...
function checkAndUnlock(achievementId: string) {
  ...
  // Confetti for rare+ achievements — dynamically imported to keep initial bundle small
  if (achievement.rarity === 'legendary' || achievement.rarity === 'epic' || achievement.rarity === 'rare') {
    import('canvas-confetti').then(({ default: confetti }) => {
      if (achievement.rarity === 'legendary') {
        confetti({
          ...
          spread: 90,
          origin: { y: 0.9, x: 0.15 },
          colors: [achievement.color, '#ffffff', '#ffd700', '#ff6b6b'],
        });
        setTimeout(
          () =>
            confetti({
...
/** Module-level action: unlock achievement without subscribing to store */
export function unlockAchievementAction(id: string) {
  checkAndUnlock(id);
}

/** Module-level action: record a prompt view without subscribing to store */
export function recordViewAction(id: string) {
...
export function usePromptStore() {
  ...
  const toggleCollectionSave = useCallback((id: string) => {
    const isSaved = _store.savedCollections.has(id);
    const next = new Set(_store.savedCollections);
    if (isSaved) next.delete(id);
    else next.add(id);
    _store = { ..._store, savedCollections: next };
    saveStore(_store);
    notifyListeners();
  }, []);
  ...
  return {
    liked: _store.liked,
    saved: _store.saved,
    savedCollections: _store.savedCollections,
    ...
    isCollectionSaved: (id: string) => _store.savedCollections.has(id),
