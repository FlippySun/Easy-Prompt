...
const CATEGORIES = [
  { id: 'writing', name: '写作创作', emoji: '✍️' },
  { id: 'coding', name: '编程开发', emoji: '💻' },
  { id: 'marketing', name: '营销文案', emoji: '📢' },
  { id: 'art', name: '图像生成', emoji: '🎨' },
  { id: 'productivity', name: '效率工具', emoji: '⚡' },
  { id: 'education', name: '学习教育', emoji: '🎓' },
  { id: 'business', name: '商业分析', emoji: '📊' },
  { id: 'life', name: '生活助手', emoji: '❤️' },
];
  ...
  return (
    <Drawer.Root open={open} onOpenChange={setOpen}>
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className={`fixed bottom-0 left-0 right-0 z-50 flex max-h-[92vh] flex-col rounded-t-2xl outline-none ${
          ...
          {/* Header */}
          <div className={`flex items-center justify-between border-b px-6 py-4 ${dm ? 'border-gray-800' : 'border-gray-100'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-6 pb-8 mx-auto max-w-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>分类 <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <ChevronDown size={14} className={`pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 ${dm ? 'text-gray-400' : 'text-gray-400'}`} />
                  </div>
                </div>
                <div>
              ...
              <div className={`flex gap-3 pt-2 border-t ${dm ? 'border-gray-800' : 'border-gray-100'}`}>
                ...
                <button
                  type="submit"
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-500 to-indigo-600 py-3 text-sm font-semibold text-white shadow-md shadow-indigo-500/25 transition-all hover:shadow-lg hover:shadow-indigo-500/40"
                >
                  <Plus size={16} />
                  提交 Prompt
                </button>
