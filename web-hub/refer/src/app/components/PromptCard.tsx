...
import { Copy, Heart, Eye, Check, ChevronDown, ChevronUp, Bot, Bookmark, BookmarkCheck, ExternalLink, ArrowLeftRight } from 'lucide-react';
...
export const CATEGORY_CONFIG: Record<string, { label: string; color: string; bg: string; darkBg: string; darkColor: string }> = {
  coding:       { label: '编程开发', color: '#3b82f6', bg: '#eff6ff',   darkBg: 'rgba(59,130,246,0.1)',  darkColor: '#60a5fa' },
  writing:      { label: '写作创作', color: '#8b5cf6', bg: '#f5f3ff',   darkBg: 'rgba(139,92,246,0.1)',  darkColor: '#a78bfa' },
  marketing:    { label: '营销文案', color: '#f59e0b', bg: '#fffbeb',   darkBg: 'rgba(245,158,11,0.1)',  darkColor: '#fbbf24' },
  art:          { label: '图像生成', color: '#ec4899', bg: '#fdf2f8',   darkBg: 'rgba(236,72,153,0.1)',  darkColor: '#f472b6' },
  productivity: { label: '效率工具', color: '#10b981', bg: '#ecfdf5',   darkBg: 'rgba(16,185,129,0.1)',  darkColor: '#34d399' },
  education:    { label: '学习教育', color: '#f97316', bg: '#fff7ed',   darkBg: 'rgba(249,115,22,0.1)',  darkColor: '#fb923c' },
  business:     { label: '商业分析', color: '#06b6d4', bg: '#ecfeff',   darkBg: 'rgba(6,182,212,0.1)',   darkColor: '#22d3ee' },
  life:         { label: '生活助手', color: '#ef4444', bg: '#fef2f2',   darkBg: 'rgba(239,68,68,0.1)',   darkColor: '#f87171' },
};
...
export function PromptCard({ prompt, index, darkMode, batchMode, selected, onSelect, showCompare }: PromptCardProps) {
  ...
  const isInCompare = store.isInCompare(prompt.id);
  const compareIsFull = store.compare.length >= 2 && !isInCompare;
  const dm = darkMode;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (glowRef.current) {
      ...
      glowRef.current.style.background =
        `radial-gradient(circle at ${gx}% ${gy}%, rgba(255,255,255,${dm ? '0.055' : '0.12'}) 0%, transparent 65%)`;
    }
  }, [dm]);
  ...
  const handleCompare = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (compareIsFull) {
      toast.info('最多可对比 2 个 Prompt', { description: '请先移除一个再添加' });
      return;
    }
    store.toggleCompare(prompt.id);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ willChange: 'transform', transform: 'perspective(800px) translateZ(0)' }}
        className={cn(
          ...
          selected
            ? dm ? 'border-indigo-500 shadow-lg shadow-indigo-500/15' : 'border-indigo-400 shadow-lg shadow-indigo-500/15'
            : '',
          !selected && (dm
      ...
      >
        ...
        <div className="flex flex-col gap-3 p-5">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {modelConfig && (
                <span
                  ...
                  style={{ borderColor: modelConfig.color + '40', color: modelConfig.color, background: modelConfig.color + '10' }}
                >
                  <Bot size={9} />
                  {modelConfig.label}
                </span>
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {showCompare && (
                <button
                  ...
                  className={cn(
                    ...
                    isInCompare
                      ? dm ? 'text-violet-400' : 'text-violet-600'
                      : compareIsFull
                        ? dm ? 'text-gray-700 cursor-not-allowed' : 'text-gray-200 cursor-not-allowed'
                        : dm ? 'text-gray-600 hover:text-violet-400' : 'text-gray-300 hover:text-violet-600'
                  )}
                >
              ...
              <button
                ...
                className={cn(
                  'rounded-lg p-1.5 transition-all',
                  isSaved
                    ? dm ? 'text-yellow-400' : 'text-yellow-500'
                    : dm ? 'text-gray-600 hover:text-yellow-400' : 'text-gray-300 hover:text-yellow-500'
                )}
          ...
          <PromptDetailDrawer prompt={prompt} darkMode={dm}>
            <button className={`text-left text-[15px] font-semibold leading-snug transition-colors group-hover:text-indigo-500 ${dm ? 'text-gray-100' : 'text-gray-900'}`}>
              {prompt.title}
            </button>
          </PromptDetailDrawer>
          ...
          <div className={`relative overflow-hidden rounded-xl border p-3.5 ${dm ? 'border-gray-700/50 bg-gray-800/60' : 'border-gray-100 bg-gray-50'}`}>
            <pre className={cn(
              'whitespace-pre-wrap break-words font-mono text-[11px] leading-relaxed transition-all',
            ...
            {!expanded && (
              <div className={`absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t ${dm ? 'from-gray-800/90 to-transparent' : 'from-gray-50 to-transparent'}`} />
            )}
            <button
              onClick={handleCopy}
              className={cn(
                ...
                copied
                  ...
                  : dm ? 'bg-gray-700 text-gray-300 hover:bg-indigo-600 hover:text-white'
          ...
          <div className="flex items-center justify-between -mt-1">
            ...
            <PromptDetailDrawer prompt={prompt} darkMode={dm}>
              <button className={`flex items-center gap-1 text-[11px] font-medium transition-colors ${dm ? 'text-gray-500 hover:text-indigo-400' : 'text-gray-400 hover:text-indigo-600'}`}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {prompt.tags.map((tag) => (
              <Link
                ...
                className={`rounded-md px-2 py-0.5 text-[11px] font-medium transition-colors ${dm
        ...
        {/* Footer */}
        <div className={`mt-auto flex items-center justify-between border-t px-5 py-3 ${dm ? 'border-gray-800 bg-gray-900/50' : 'border-gray-100 bg-gray-50/50'}`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 text-[11px] font-medium">
              ...
              <span className={`flex items-center gap-1 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
                <Eye size={11} />{formatCount(prompt.views)}
              </span>
              <span className={`flex items-center gap-1 ${dm ? 'text-gray-500' : 'text-gray-400'}`}>
                <Copy size={11} />{formatCount(prompt.copies)}
              </span>
            </div>
          </div>
          <button
            ...
            className={cn(
              ...
              isLiked
                ? dm ? 'bg-red-500/10 text-red-400' : 'bg-red-50 text-red-500'
                : dm ? 'text-gray-500 hover:bg-gray-800 hover:text-red-400' : 'text-gray-400 hover:bg-red-50 hover:text-red-500',
            )}
          >
