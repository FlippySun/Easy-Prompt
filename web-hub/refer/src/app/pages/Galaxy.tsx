...
export function Galaxy() {
      ...
      Object.entries(CLUSTER_POS).forEach(([cat, pos]) => {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const grad = ctx.createRadialGradient(
            pos.x + i * 30, pos.y + i * 20, 0,
            pos.x + i * 30, pos.y + i * 20,
            280 * pulseFactor
          );
  ...
  const findHoveredStar = useCallback((worldX: number, worldY: number) => {
  ...
  useEffect(() => {
    const fn = () => { isDraggingRef.current = false; };
  ...
  // Escape to exit
  useEffect(() => {
    const fn = (e: KeyboardEvent) => { if (e.key === 'Escape') navigate('/'); };
    ...
    <div className="fixed inset-0 z-50 overflow-hidden bg-[#020210]">
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col justify-between p-5 md:p-7">
        ...
        <div className="flex items-start justify-between">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="bg-gradient-to-r from-indigo-400 via-violet-400 to-fuchsia-400 bg-clip-text text-xl font-bold text-transparent">
                  Prompt 银河
                </h1>
        ...
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="pointer-events-auto flex flex-col gap-1.5">
            <button onClick={() => { zoomRef.current = Math.min(zoomRef.current * 1.25, 3); }}
            ...
            <button onClick={() => { zoomRef.current = Math.max(zoomRef.current * 0.8, 0.12); }}
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-700/60 bg-gray-950/70 text-gray-400 backdrop-blur-md transition-all hover:border-indigo-500/50 hover:text-indigo-400">
              <ZoomOut size={15} />
            </button>
          ...
          >
            <div className="flex items-center gap-2 rounded-2xl border border-white/8 bg-gray-950/70 px-5 py-2.5 backdrop-blur-md">
              ...
              <span className="text-[12px] text-gray-400">
                拖拽移动 · 滚轮缩放 · 点击星球查看 Prompt · <kbd className="rounded border border-gray-700 bg-gray-800 px-1 font-mono text-[10px]">Esc</kbd> 返回
              </span>
            </div>
          </motion.div>
        )}
      ...
      {selectedPrompt && (
        <PromptDetailDrawer
          ...
          onExternalOpenChange={(o) => { setDrawerOpen(o); if (!o) setSelectedPrompt(null); }}
        />
      )}
    </div>
  );
