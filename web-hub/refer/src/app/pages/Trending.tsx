...
const CATEGORY_COLORS: Record<string, string> = {
  writing: '#8b5cf6', coding: '#3b82f6', marketing: '#f59e0b',
  art: '#ec4899', productivity: '#10b981', education: '#f97316',
  business: '#06b6d4', life: '#ef4444',
};

const CATEGORY_LABELS: Record<string, string> = {
  writing: '写作创作', coding: '编程开发', marketing: '营销文案',
  art: '图像生成', productivity: '效率工具', education: '学习教育',
  business: '商业分析', life: '生活助手',
};

// Mock daily trend data for the last 14 days
function generateDailyData() {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    days.push({
      date: label,
      浏览量: Math.floor(Math.random() * 8000 + 4000),
      复制次数: Math.floor(Math.random() * 3000 + 1000),
      点赞数: Math.floor(Math.random() * 1500 + 500),
    });
  }
  return days;
}
...
export function Trending() {
  ...
  const topByLikes = useMemo(() =>
    [...MOCK_PROMPTS].sort((a, b) => b.likes - a.likes).slice(0, 10),
    []
  );
  const topByCopies = useMemo(() =>
    [...MOCK_PROMPTS].sort((a, b) => b.copies - a.copies).slice(0, 5),
    []
  );

  const categoryData = useMemo(() => {
    const counts = MOCK_PROMPTS.reduce((acc, p) => {
    ...
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div>
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-orange-400 to-rose-500">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { icon: TrendingUp, label: '精选 Prompt', value: MOCK_PROMPTS.length, color: '#6366f1', suffix: '个' },
          { icon: Eye, label: '总浏览量', value: totalStats.views, color: '#3b82f6', suffix: '' },
          { icon: Copy, label: '总复制次数', value: totalStats.copies, color: '#10b981', suffix: '' },
          { icon: Heart, label: '总点赞数', value: totalStats.likes, color: '#ef4444', suffix: '' },
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className={`mt-1 text-2xl font-bold ${dm ? 'text-white' : 'text-gray-900'}`}>
                  {typeof value === 'number' ? formatCount(value) : value}{suffix}
                </p>
              </div>
            ...
            <div className="mt-3 flex items-center gap-1 text-[11px] font-medium text-green-500">
              <ArrowUp size={10} />
              <span>较上周 +{Math.floor(Math.random() * 20 + 5)}%</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className={`col-span-2 ${cardBase}`}>
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className={`text-sm font-semibold ${dm ? 'text-gray-200' : 'text-gray-800'}`}>热门 Prompt 排行（点赞数）</h3>
          </div>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={barChartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fill: axisColor, fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => formatCount(v)} />
              <Tooltip
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {categoryData.map(({ name, color, value }) => (
              <div key={name} className="flex items-center gap-1">
                ...
                <span className={`text-[10px] ${dm ? 'text-gray-400' : 'text-gray-500'}`}>{name}({value})</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className={cardBase}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={dailyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="浏览量" stroke="#6366f1" strokeWidth={2} fill="url(#colorViews)" dot={false} />
            <Area type="monotone" dataKey="复制次数" stroke="#10b981" strokeWidth={2} fill="url(#colorCopies)" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
          ...
          <div className="flex flex-col gap-2">
            {topByCopies.map((prompt, i) => (
              <PromptDetailDrawer key={prompt.id} prompt={prompt} darkMode={dm}>
                <button className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`truncate text-sm font-medium ${dm ? 'text-gray-200' : 'text-gray-800'}`}>{prompt.title}</p>
                    <p className={`text-[11px] ${dm ? 'text-gray-500' : 'text-gray-400'}`}>{formatCount(prompt.copies)} 次复制</p>
                  </div>
            ...
            {topByLikes.slice(0, 5).map((prompt, i) => (
              <PromptDetailDrawer key={prompt.id} prompt={prompt} darkMode={dm}>
                <button className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors ${
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`truncate text-sm font-medium ${dm ? 'text-gray-200' : 'text-gray-800'}`}>{prompt.title}</p>
                    <p className={`text-[11px] ${dm ? 'text-gray-500' : 'text-gray-400'}`}>{formatCount(prompt.likes)} 个点赞</p>
                  </div>
