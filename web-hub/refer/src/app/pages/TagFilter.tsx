...
export function TagFilter() {
  ...
  useEffect(() => {
    if (decodedTag) store.unlockAchievement('tag_explorer');
  }, [decodedTag]);

  const filteredPrompts = useMemo(() => {
    let prompts = MOCK_PROMPTS.filter(p => p.tags.some(t => t === decodedTag));
  ...
  const relatedTags = useMemo(() => {
    ...
    filteredPrompts.forEach(p => {
      p.tags.filter(t => t !== decodedTag).forEach(t => {
        related[t] = (related[t] || 0) + 1;
      });
    });
    return Object.entries(related).sort((a, b) => b[1] - a[1]).slice(0, 10);
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div className="flex flex-col gap-4">
        <Link to="/" className={`flex w-fit items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-medium transition-colors ${dm ? 'border-gray-800 text-gray-400 hover:bg-gray-800 hover:text-gray-200' : 'border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-700'}`}>
          <ArrowLeft size={13} /> 返回首页
        </Link>

        <div className={`flex flex-col gap-4 overflow-hidden rounded-2xl border p-6 ${dm ? 'border-gray-800 bg-gray-900' : 'border-gray-200 bg-white'}`}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {relatedTags.map(([tag, count]) => (
                  ...
                  >
                    <Hash size={9} />
                    {tag}
                    <span className={`ml-0.5 rounded-md px-1 text-[9px] ${dm ? 'bg-gray-700 text-gray-500' : 'bg-gray-200 text-gray-400'}`}>{count}</span>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 transition-colors ${
                sortBy === id
                  ...
                  : dm ? 'border-gray-700 bg-gray-800 text-gray-400 hover:bg-gray-700' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'
          ...
          <ResponsiveMasonry columnsCountBreakPoints={{ 0: 1, 640: 1, 768: 2, 1100: 2, 1280: 3 }}>
            <Masonry gutter="16px">
              {filteredPrompts.map((prompt, index) => (
                <PromptCard key={prompt.id} prompt={prompt} index={index} darkMode={dm} showCompare />
              ))}
            </Masonry>
          </ResponsiveMasonry>
        </motion.div>
          ...
          {ALL_TAGS.map(({ name, count }) => {
            ...
            return (
              ...
              >
                #{name}
                {count > 1 && (
                  <span className={`rounded px-1 text-[9px] ${isActive ? 'bg-indigo-600 text-indigo-100' : dm ? 'bg-gray-700 text-gray-500' : 'bg-gray-200 text-gray-400'}`}>{count}</span>
                )}
              </Link>
            );
          })}
