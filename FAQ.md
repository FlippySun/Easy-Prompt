...
### Q: Fast 和 Deep 模式有什么区别？

**A:** 当前版本中，Fast / Deep 都会使用你当前配置的模型与 API 接口，区别只在于**第二步生成的输出深度**：

- **Fast**：结果更精炼，通常更快，适合日常快速增强
- **Deep**：结果更完整，通常更慢，适合复杂任务、复合任务或需要更充分边界条件时

> 💡 重要：Fast / Deep **不会自动切换模型，也不会修改请求接口**。尤其在浏览器扩展中，这样做是为了尽量避免真实浏览器环境里的跨域 / 超时类回归。
...
2. **API 服务慢**: 复合任务可能需要 30s+，请耐心等待；如任务很复杂可先试 `Fast`
3. **配置错误**: Base URL 是否正确

### Q: 浏览器扩展提示跨域 / CORS / 超时怎么办？

**A:** 浏览器扩展是直接从浏览器环境发请求，比 VSCode / IntelliJ / Web 在线版更容易受到网络、服务商策略和浏览器限制影响。建议按顺序排查：

1. 先确认当前网络是否稳定，再重试一次
2. 检查你配置的 API 服务是否支持浏览器直接访问
3. 确认 API Host / Path / Model 没有填错
4. 复杂任务优先先试 `Fast`，看是否只是生成时间过长
5. 如果只有浏览器扩展失败，而 Web/VSCode 正常，优先怀疑浏览器直连环境差异

> 当前版本的 Browser Fast / Deep 已刻意保持“**不改变请求形状**”，因此若浏览器端仍报跨域/超时，通常更可能是网络或浏览器直连环境问题，而不是模式本身改了请求接口。
...
4. **优化建议**: 使用"指定场景"跳过意图识别可节省 2-5s；复杂任务可优先试 `Fast`
